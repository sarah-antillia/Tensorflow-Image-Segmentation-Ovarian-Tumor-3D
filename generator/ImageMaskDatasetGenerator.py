...
class ImageMaskDatasetGenerator:
  ...
  def create_mask_files(self, mask_file, output_masks_dir, index):
  ...
  def create_image_files(self, image_file, output_images_dir, index):
  ...
  def generate(self, input_images_dir, input_masks_dir, 
    ...
    for index in range(1000):
      ...
      mask_file  = input_masks_dir + str(index) + binary + ".png"

      image_file = input_images_dir + str(index) + ".JPG"
      if os.path.exists(mask_file) and os.path.exists(image_file):
        self.create_mask_files(mask_file,   output_masks_dir, index)
        self.create_image_files(image_file, output_images_dir, index)
      else:
        break
...
if __name__ == "__main__":
  try:
    ...
    input("---- Generaate type {} OK?".format(type))
